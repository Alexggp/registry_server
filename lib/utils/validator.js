...
(function (exports) {
  ...
  function validateObject(object, schema, options, errors) {
    let props; const allProps = Object.keys(object);
    const visitedProps = [];
    ...
    if (undefined !== schema.additionalProperties) {
      let i; let
        l;
      const unvisitedProps = allProps.filter(k => visitedProps.indexOf(k) === -1);
  ...
  function validateProperty(object, value, property, schema, options, errors) {
    ...
    if (options.cast) {
      if ((schema.type === 'integer' || schema.type === 'number') && value == +value) {
        value = +value;
    ...
    if (isArray(schema.dependencies)) {
      for (let i = 0, l = schema.dependencies.length; i < l; i++) {
    ...
    checkType(value, schema.type, (err, type) => {
      if (err) return error('type', property, typeof value, schema, errors);
      constrain('conform', value, (a, e) => e(a, object));
      switch (type || (isArray(value) ? 'array' : typeof value)) {
        case 'string':
          constrain('allowEmpty', value, (a, e) => (e || a !== ''));
          constrain('minLength', value.length, (a, e) => a >= e);
          constrain('maxLength', value.length, (a, e) => a <= e);
          constrain('pattern', value, (a, e) => {
            e = typeof e === 'string'
              ? e = new RegExp(e)
              : e;
            return e.test(a);
        ...
        case 'integer':
        case 'number':
          constrain('minimum', value, (a, e) => a >= e);
          constrain('maximum', value, (a, e) => a <= e);
          constrain('exclusiveMinimum', value, (a, e) => a > e);
          constrain('exclusiveMaximum', value, (a, e) => a < e);
          constrain('divisibleBy', value, (a, e) => {
            let multiplier = Math.max((a - Math.floor(a)).toString().length - 2, (e - Math.floor(e)).toString().length - 2);
            multiplier = multiplier > 0 ? Math.pow(10, multiplier) : 1;
            return (a * multiplier) % (e * multiplier) === 0;
          });
          break;
        case 'array':
          constrain('items', value, (a, e) => {
            for (let i = 0, l = a.length; i < l; i++) {
              validateProperty(object, a[i], property, e, options, errors);
            }
            return true;
          });
          constrain('minItems', value, (a, e) => a.length >= e);
          constrain('maxItems', value, (a, e) => a.length <= e);
          constrain('uniqueItems', value, (a) => {
            const h = {};
            for (let i = 0, l = a.length; i < l; i++) {
  ...
  function error(attribute, property, actual, schema, errors) {
    const lookup = {
      expected: schema[attribute], actual, attribute, property,
    };
    let message = schema.messages && schema.messages[attribute] || schema.message || validate.messages[attribute] || 'no default message';
    message = message.replace(/%\{([a-z]+)\}/ig, (_, match) => lookup[match.toLowerCase()] || '');
    errors.push({
      attribute,
      property,
